...
uniform_int_distribution<int> distributionRW(0, 100);
...
int main(int argc, char *argv[]) {
    ...
    ///output
    char filename[200];
    sprintf(filename, "test_data_SFN_Float_%dN_%dE.txt", NE, ei);
    FILE *fp = fopen(filename, "wb");
    char float_tail_str[5];
    int float_tail_int;
    for (auto e: E) {
        ...
        if (distributionRW(rng) > 20){
            float_tail_int = distributionRW(rng);
            if (float_tail_int > 10 && (float_tail_int % 10 == 0)){
                sprintf(float_tail_str, ".%02d", float_tail_int);
                fwrite(float_tail_str, 2, sizeof(char), fp);
            }
            else{
                sprintf(float_tail_str, ".%02d", float_tail_int);
                fwrite(float_tail_str, 3, sizeof(char), fp);
            }
        } else if (distributionRW(rng) > 95){
            sprintf(float_tail_str, ".0");
            fwrite(float_tail_str, 2, sizeof(char), fp);
        }
        fwrite("\n", 1, sizeof(char), fp);
    }
